...
export function TripSetupDrawer({ isOpen, onClose, tripType, isDemo = false }: Props) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#132432]/55 p-0 backdrop-blur-sm sm:items-center sm:p-6"><section className="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[30px] bg-[#fffdfa] shadow-2xl sm:rounded-[30px]">
    ...
    <footer className="border-t border-[#eadfd7] bg-white px-5 py-4 sm:px-8">{stage === "discussion" && <div className="flex gap-2"><input value={message} onChange={e => setMessage(e.target.value)} placeholder="Tell me anything about your trip…" className="min-w-0 flex-1 rounded-xl border border-[#eadfd7] px-4 text-sm outline-none focus:border-[#ff6b3d]" /><button className="flex size-11 items-center justify-center rounded-xl bg-[#ff6b3d] text-white"><Send className="size-4" /></button></div>}{stage === "finalizing" && <div className="flex gap-2"><button onClick={() => setStage("discussion")} className="flex items-center gap-2 rounded-xl border border-[#eadfd7] px-4 py-3 text-sm font-extrabold text-[#39464d]"><ArrowLeft className="size-4" /> Back to chat</button><button onClick={startChat} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#ff6b3d] px-4 py-3 text-sm font-extrabold text-white">Confirm plan <ArrowRight className="size-4" /></button></div>}{stage === "booking" && <button onClick={() => setStage("manage")} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#ff6b3d] px-4 py-3 text-sm font-extrabold text-white">Pay RM 1,289 securely <ArrowRight className="size-4" /></button>}{stage === "manage" && <button onClick={openItinerary} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#eadfd7] px-4 py-3 text-sm font-extrabold text-[#39464d]">Open full trip itinerary <ArrowRight className="size-4" /></button>}</footer>
    {qrTitle && <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#132432]/45 p-5"><div className="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl relative"><button onClick={() => setQrTitle(null)} className="absolute top-6 left-6 text-[#718088] hover:text-[#29363d]"><ArrowLeft className="size-5" /></button><button onClick={() => setQrTitle(null)} className="absolute top-6 right-6 text-[#718088] hover:text-[#29363d]"><X className="size-5" /></button><QrCode className="mx-auto mt-3 size-44 text-[#29363d]" /><h3 className="mt-4 text-lg font-extrabold text-[#29363d]">{qrTitle}</h3><p className="mt-1 text-sm text-[#718088]">Mock QR pass · Ready to scan</p><button onClick={() => setQrTitle(null)} className="mt-5 w-full rounded-xl bg-[#ff6b3d] py-3 text-sm font-extrabold text-white">Done</button></div></div>}
  </section></div>;
}
...
function Booking({ selectedFlight, setSelectedFlight, selectedHotel, setSelectedHotel, onPay }: { selectedFlight: string; setSelectedFlight: (v: string) => void; selectedHotel: string; setSelectedHotel: (v: string) => void; onPay: () => void }) { return <><div className="mb-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#ff6b3d]">One checkout</p><h3 className="mt-1 text-2xl font-extrabold text-[#29363d]">Everything in one place</h3></div><div className="space-y-3"><BookingRow icon={<Plane />} title="KUL → KIX · 12 Oct" options={["Malaysia Airlines · RM 1,289", "AirAsia X · RM 899"]} selected={selectedFlight} setSelected={setSelectedFlight} /><div className="flex items-center justify-between rounded-2xl bg-[#fff2ed] p-4"><div><p className="text-xs font-bold text-[#899397]">Total trip bundle</p><p className="text-xl font-extrabold text-[#29363d]">RM 1,289 <span className="text-xs font-bold text-[#899397]">incl. fees</span></p></div><CreditCard className="size-6 text-[#ff6b3d]" /></div></div></> }
